...
class Array(object):
    ...
    def __eq__(self, other):
        ...
        if self_length != other_length:
            return 0
        if isinstance(self, (cint, regint)) and isinstance(other, (sint, sregint)):
            return other == self
        if self.value_type == sint or self.value_type == sregint:
            result = self.__secret_vector_equals_pr(other)
        elif self.value_type == cint:
            result = self.__cint_vector_equals(other)
        elif self.value_type == sfix:
            result = self.sfix_vector_equals(other)
        else:
            raise CompilerError('Mismatching types')
    ...
    def __load_vector_in_mem(self):
        length = len(self)
        vector = self.value_type(size=length)
        if self.value_type == sint:
            vldms(length, vector, self.address)
        elif self.value_type == cint:
            vldmc(length, vector, self.address)
        elif self.value_type == sregint:
            vldmsint(length, vector, self.address)
        elif self.value_type == regint:
            vldmint(length, vector, self.address)
        else:
            raise CompilerError('Mismatching types')

        return vector

    def __store_from_mem(self, vector):
        length = len(self)
        array = Array(length, self.value_type)
        if self.value_type == sint:
            vstms(length, vector, array.address)
        elif self.value_type == sregint:
            vstmsint(length, vector, array.address)
        else:
            raise CompilerError('Mismatching types')
    ...
    @staticmethod
    def generate_random_clear_array(length):
        random_vector = cint(size=length)
        vrandc(length, random_vector)
        random = Array(length, cint)
        vstmc(length, random_vector, random.address)
        return random

    def __product_summation(self, random):
        summation = sint(0)
        summation.store_in_mem(5000)

        @library.for_range(len(self))
        def f(i):
            accumulator = sint.load_mem(5000)
            accumulator += random[i] * sint(self[i])
            accumulator.store_in_mem(5000)

        summation = sint.load_mem(5000)
        return self.value_type(summation)

    def __secret_vector_equals_pr(self, other):
        vector_a = self.__load_vector_in_mem()
        vector_b = other.__load_vector_in_mem()
        difference_vector = vector_a - vector_b
        difference_array = self.__store_from_mem(difference_vector)

        random = Array.generate_random_clear_array(len(self))
        summation = difference_array.__product_summation(random)
        return summation == 0

    def __cint_vector_equals(self, other):
        self_length = len(self)
        other_length = len(other)
        a = cint(size=self_length)
        b = cint(size=other_length)
        vldmc(self_length, a, self.address)
        vldmc(other_length, b, other.address)

        result = cint_memory_vector_equals(a, b, self_length)
        return result

    def sfix_vector_equals(self, B):
        from library import if_then, end_if
        ...
        vldms(A_length, a_f, self.address)
        ...
        D = Array(A_length, cint)
        vstmc(A_length, d_f_cint, D.address)

        are_equal = MemValue(cint(1))
        ...
        return cint(are_equal) == cint(1)


def cint_memory_vector_equals(a, b, output_length):
    from library import if_then, end_if

    c = a == b
    C = Array(output_length, cint)
    d = cint(size=output_length)
    vconvint(output_length, d, c)

    vstmc(output_length, d, C.address)
    result = sum(z for z in C) - output_length + 1

    result.store_in_mem(1000)

    if_then(result != 1)
    res = cint(0)
    res.store_in_mem(1000)
    end_if()

    return cint.load_mem(1000)
...
class Matrix(object):
    ...
    def to_vector(self):
        return Array(self.columns * self.rows, self.value_type, self.address)
...
class _fixMatrix(Matrix):
    def __init__(self, rows, columns, address=None):
        ...
        self.columns = columns
        self.address = address
        Matrix.__init__(self, rows, columns, self.value_type, address)
    ...
    #    return self.value_type.load_int(self[index])
...
class sfixMatrix(_fixMatrix):
    ...
    def __init__(self, rows, columns, address=None):
        _fixMatrix.__init__(self, rows, columns, address=None)


## Specialized types for PPML

##
# class that implements CKRRSW20 mult for convolutions
# multiplications here use the triples instruction
class convMatrix(Matrix):
    def __init__(self, rows, columns, value_type=sint, address=None):
        super(Matrix, self).__init__(rows, columns, value_type)
