...
instructions! {
 "Load/store" {
  ...
  "Addition" {
  SUMS & 0x2D & (dest: sw, value: s, n: int) & & r##"SUMS si sj \newline
                                              Sums all secret registers si=sum[s1+...+sn] with size n."## & ""
  SUMC & 0x2E & (dest: cw, value: c, n: int) & & r##"SUMC ci cj \newline
                                              Sums all secret registers ci=sum[c1+...+cn] with size n."## & ""
  ...
  OGC  & 0xDF & (i: int, j: int) & mem_read mem_write & r##"OGC i \newline
                              This calls the Garbled Circuit j times with circuit number i."## & r##"c2"##
  LOADGC  & 0xE9 & (i: int, j: int) & mem_read mem_write & r##"LOAD_GC i \newline
                              This loads the Garbled Circuit j times with circuit number i in memory."## & r##"c2"##
  EGC  & 0xE8 & (i: int) & mem_read mem_write & r##"EGC i \newline
                              This calls the Evaluation of a Garbled Circuit with circuit number i."## & r##"c2"##
