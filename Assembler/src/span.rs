...
impl<'a> Span {
    pub fn new(file_index: u32, start: u64, end: u64, expansion: Expansion) -> Self {
        Self {
            file_index,
            start,
            end,
    ...
    pub fn lines(cx: &'a Compiler, file_index: u32) -> impl Iterator<Item = Span> + 'a {
        let file_str = cx.files.get(file_index.try_into().unwrap()).unwrap();
        file_str.lines().map(move |line| {
            let start = substr_offset(line, file_str).unwrap();
            let start = u64::try_from(start).unwrap();
            Span {
                file_index,
                start,
                end: start + u64::try_from(line.len()).unwrap(),
                expansion: Expansion::None,
            }
        })
    }
    ...
    pub fn trim(mut self, cx: &Compiler) -> Self {
        ...
        if let Some(pos) = first {
            self.start += u64::try_from(pos).unwrap();
        } else {
            return self.start();
        }
        let last = self.snippet(cx).rfind(|c: char| !c.is_whitespace());
        if let Some(pos) = last {
            self.end = self.start + u64::try_from(pos).unwrap() + 1;
        } else {
            unreachable!()
        }
        self
