...
{
  vals.resize(maximum_number_of_ssl_connections);
  macs.resize(maximum_number_of_ssl_connections);
  sha256.resize(maximum_number_of_ssl_connections);
  for (unsigned int i= 0; i < maximum_number_of_ssl_connections; i++)
    {
...
Open_Protocol::~Open_Protocol()
{
  for (unsigned int i= 0; i < number_of_used_connections; i++)
    {
      if (open_cnt[i] > 0)
        {
                                    ...
                                    Player &P, int connection, bool verbose)
{
  ...
  //this part is done to work with the batched version of PO_Open_Stop
  const int number_of_original_connections = 3;
  if (connection >= number_of_original_connections)
    {
      RunOpenCheck(P, "", connection);
    }
}
...
void Open_Protocol::Increase_Number_Of_Used_Connections(unsigned int connections){
  number_of_used_connections += connections;
}