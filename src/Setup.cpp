...
void generate_empty_choicebits_file_for_players(unsigned int number_of_players){
    for(unsigned int i = 0; i < number_of_players; i++) {
        ofstream choicebitsOutput(Choicebits::get_config_file_name(i));
    }
}
  ...
  switch (lsss)
    {
      case Full:
        {
          generate_empty_choicebits_file_for_players(n);
          int answer= -1;
