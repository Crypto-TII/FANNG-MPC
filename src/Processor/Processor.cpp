...
template<typename T>
vector<T> fill_batch(vector<T> original_vector, unsigned int inserted_values)
{
  vector<T> batched_vector;
  batched_vector.resize(open_values_workaround_batch_size);
  batched_vector.assign(original_vector.begin() + inserted_values, original_vector.begin() + inserted_values + open_values_workaround_batch_size);
...
void Processor<SRegint, SBit>::POpen_Start_Batched(const vector<int> &reg, int size, Player &P)
{
  ...
  bool are_there_enough_connections= number_of_batches < maximum_number_of_ssl_connections - number_of_original_connections;
...
void Processor<aBitVector, aBit>::garble_circuit(unsigned int circuit_number, unsigned int number_of_times, Player &P)
{
  ...
  Choicebits choicebits= Choicebits::read_from_config_file(P.whoami());
...
template<>
void Processor<aBitVector2, Share2>::garble_circuit([[maybe_unused]] unsigned int circuit_number, [[maybe_unused]] unsigned int number_of_times,  [[maybe_unused]] Player &P)
{
  throw Instruction_not_supported_exception();
}

template<>
void Processor<aBitVector, aBit>::load_garbled_circuit(unsigned int circuit_number, unsigned int number_of_times, Player &P)
{
  Choicebits choicebits= Choicebits::read_from_config_file(P.whoami());
  vector<Base_Garbled_Circuit> read_circuits= gc_storage->read(number_of_times, choicebits, circuit_number, P.whoami());
  circuits.insert(circuits.end(), read_circuits.begin(), read_circuits.end());
}

template<>
void Processor<aBitVector2, Share2>::load_garbled_circuit([[maybe_unused]] unsigned int circuit_number, [[maybe_unused]] unsigned int number_of_times, [[maybe_unused]] Player &P)
{
  throw Instruction_not_supported_exception();
}

template<>
void Processor<aBitVector, aBit>::evaluate_garbled_circuit(unsigned int circuit_number, Player &P)
{
  Global_Circuit_Store.check(circuit_number);
...
template<>
void Processor<aBitVector2, Share2>::evaluate_garbled_circuit([[maybe_unused]] unsigned int circuit_number, [[maybe_unused]] Player &P)
{
  throw Instruction_not_supported_exception();
}
