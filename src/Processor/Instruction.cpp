  ...
  switch (opcode)
    {
      ...
      default:
        ...
        os << "Invalid instruction :'( " << hex << showbase << opcode << " at " << dec
           << pos << " the opcode is: " << opcode << " the end";
    ...
    offline_control_data &OCD) const
{
  ...
  if (opcode == TRIPLE || opcode == SQUARE || opcode == BIT || opcode == DABIT || opcode == PRIVATE_OUTPUT ||
      opcode == PRIVATE_INPUT || opcode == STARTOPEN || opcode == STOPOPEN || opcode == MPRIVATE_OUTPUT ||
...
#pragma GCC diagnostic pop
              ...
              case LDSBIT:
                ...
                break;
              case ADDSINT:
                Proc.get_srint_ref(r[0]).add(Proc.read_srint(r[1]), Proc.read_srint(r[2]), P,
              ...
              case ADDSINTC:
                ...
                break;
              case SUBSINT:
                Proc.get_srint_ref(r[0]).sub(Proc.read_srint(r[1]), Proc.read_srint(r[2]), P,
                                             Proc.online_thread_num);
                break;
              case SUBSINTC:
              ...
              case SUBCINTS:
                ...
                break;
              case MULSINT:
                Proc.get_srint_ref(r[0]).mul(Proc.read_srint(r[1]), Proc.read_srint(r[2]), P,
                                             Proc.online_thread_num);
                break;
              case MULSINTC:
              ...
              case MUL2SINT:
                ...
                break;
              case DIVSINT:
                Proc.get_srint_ref(r[0]).div(Proc.read_srint(r[1]), Proc.read_srint(r[2]), P,
                                             Proc.online_thread_num);
                break;
              case SHLSINT:
              ...
              case OGC:
                ...
                break;
              case LOADGC:
                Proc.load_garbled_circuit(n, m, P);
                break;
              case EGC:
              ...
              case LOADCT:
                ...
                break;
                case CTRIPLE: {
                  vector<Share> values= Proc.obtain_next_matrix_triple(P);
                  ...
                  for (unsigned int k= 0; k < invocations; k++)
                    {
                      ...
                      if (invocations - 1 == k)
                        ...
                        {
                          values= Proc.obtain_next_matrix_triple(P);
                        }
                    }
                  break;
                }
                case CT_DYN: {
                  ...
                  vector<Share> values= Proc.obtain_matrix_triple_by_triple_id(m, P);
                  for (unsigned int j= 0; j < c_start.size(); j++)
                    {
