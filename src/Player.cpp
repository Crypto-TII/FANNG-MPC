...
void main_logic(vector<int> pns, vector<int> minimums, vector<int> maximums, unsigned int maxI, unsigned int my_number,
                int verbose, int fhefacts, string progname, string memtype, unsigned int portnumbase, bool OT_disable,
                std::function<Input_Output_Base *(void *)> api_constructor, void *api_handle);

extern "C" int run_scale_from_rust(unsigned int portnumbase, char *memtype, int verbose, int fhe_factories,
                                   unsigned int *pns, unsigned int pns_n, int *min, int *max, unsigned char ot_disable,
                                   unsigned int max_i, unsigned int my_number, const char *progname,
                                   void *api_constructor, void *api_handle)
{
    vector<int> pns_vec;
    ...
    try
    {
        ...
        main_logic(pns_vec, min_vec, max_vec, max_i, my_number, verbose, fhe_factories, progname_cpp, memtype_cpp,
                   portnumbase, ot_disable, api_function, api_handle);
        return 0;
    }
...
int main(int argc, const char *argv[])
{
    ...
    main_logic(pns, minimums, maximums, maxI, my_number, verbose, fhefacts, progname, memtype, portnumbase, OT_disable,
               nullptr, nullptr);
}
#endif
                ...
                std::function<Input_Output_Base *(void *)> api_constructor, void *api_handle)
{
    ...
    if (Share::SD.Etype == HSS)
    {
        ...
        Run_Scale(my_number, no_online_threads, PTD, pk, sk, MacK, ctx, portnum, SD, machine, OCD, number_FHE_threads,
                  OT_disable, verbose);
