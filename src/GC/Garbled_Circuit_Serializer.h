...
public:
  ...
  template<class Archive>
  void serialize(Archive &ar){
    ar(CEREAL_NVP(mmo_dto), CEREAL_NVP(lambda_dto), CEREAL_NVP(k_dto), CEREAL_NVP(gab_dto), CEREAL_NVP(one_label_dto));
  }
...
public:
  ...
  static vector<vector<vector<vector<gf2n>>>> text_4D_matrix_as_4D_gf2n_matrix(const vector<vector<vector<vector<string>>>> &values);

  static vector<vector<vector<vector<string>>>> g2fn_4D_matrix_as_4D_text_matrix(const vector<vector<vector<vector<gf2n>>>> &values);

  static void serialize_garble_circuit_to_json_file(const Base_Garbled_Circuit& garbled_circuit, const string& fileName);

  static string serialize_garble_circuit_to_binary(const Base_Garbled_Circuit& garbled_circuit);

  static string serialize_garble_circuit_to_json(const Base_Garbled_Circuit& garbled_circuit);

  static string serialize_to_base64(const Base_Garbled_Circuit& garbled_circuit);
  ...
  static Base_Garbled_Circuit deserialize_from_base64(const string& circuit);
};
